...
(defmacro 比较 (&optional &body body)"【比较/eql】比较两个参数的是否相等"`(eql ,@body))
...
(defmacro 调用函数 (&optional &body body)"【调用函数/apply】接受一个函数和实参列表，返回把传入函数应用在实参列表的结果，可以是任意数量的实参，但最后一个要是列表"`(apply ,@body))
...
(defmacro 分支判断 (&optional &body body)"【分支判断/cond】后面跟多个判断条件，条件后是要执行的表达式"`(cond ,@body))
